...
static int build_tree(int *freqs, BiTree **tree) {
    ...
    *tree = NULL;
    ...
    for (c = 1; c <= size - 1; c++) {
        ...
        /* Extract the two trees whose root nodes have the smallest
         * frequencies. */
        ...
        /* Insert the merged tree into the priority queue and free the
         * others. */
...
static void build_table(BiTreeNode *node, unsigned short code,
    ...
    if (!bitree_is_eob(node)) {
        ...
        if (!bitree_is_eob(bitree_right(node))) {
            ...
            build_table(bitree_right(node), (code << 1) | 0x0001, size + 1,
                    table);
        }
    ...
    while (ipos < size) {
        ...
        for (i = 0; i < table[c].size; i++) {
            ...
            if (opos % 8 == 0) {
                ...
                if ((temp = (unsigned char *) realloc(comp, (opos / 8) + 1))
                        == NULL) {
    ...
    *original = orig = NULL;
    ...
    while (opos < size) {
        ...
        if (bitree_is_eob(bitree_left(node))
            ...
            /* Write the symbol in the leaf node to the buffer of original
             * data. */
            if (opos > 0) {
